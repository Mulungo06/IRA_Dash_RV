...
def process_influenza(sresult_file, demo_file, progress_callback=None, target_sheet=None):
    ...
    # SARS-CoV-2
    # A secção no SResult chama-se "RT-PCR 2019 CORONAVIRUS" ou "RT-PCR SARS-COV2"
    # A 1ª dd-mm-aaaa após a secção = data de entrada no teste (col 63 no ficheiro)
    # A 2ª dd-mm-aaaa após a secção = data de testagem/validação (col 66 no ficheiro)
    _sars_sec = r"RT-PCR 2019 CORONA|RT-PCR SARS-COV|RT-PCR SARS|RTRP-SARS"
    i_sars_test  = find_col(_sars_sec, r"^dd-mm-aaaa$", 1)
    i_sars_valid = find_col(_sars_sec, r"^dd-mm-aaaa$", 2)
    i_sars_res   = (find_col(r"RTCO2|RTRP-SARS", r"^SARS-CoV-2$|^Result$|CV19R") or
                   find_col(r"RTCO2", r"Result"))

    # RSV
    # A secção no SResult chama-se "VÍRUS SINCICIAL RESPIRATÓRIO" (com caracteres especiais)
    _rsv_sec = r"RT-PCR RSV|RTRP-RSV|RSV-RSVRS|V.RUS SINCIC|VIRUS SINCIC|V[\x00-\xff]RUS SINCIC"
    i_rsv_test  = find_col(_rsv_sec, r"^dd-mm-aaaa$", 1)
    i_rsv_valid = find_col(_rsv_sec, r"^dd-mm-aaaa$", 2)
