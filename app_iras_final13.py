...
def process_influenza(sresult_file, demo_file, progress_callback=None, target_sheet=None):
    ...
    # ── TRL automático (dias entre data_de_entrada e data_de_validacao) ────────
    # TRL = data_de_validacao_virus − data_de_entrada
    def calc_trl(data_entrada_dmy, data_valid_dmy):
        """Ambas as datas em DD/MM/AAAA. Devolve número de dias (int) ou None."""
        try:
            if not data_entrada_dmy or not data_valid_dmy:
                return None
            s_ent   = str(data_entrada_dmy).strip()
            s_valid = str(data_valid_dmy).strip()
            if s_ent in ('', 'None', 'nan') or s_valid in ('', 'None', 'nan'):
                return None
            d_ent   = datetime.strptime(s_ent,   '%d/%m/%Y')
            d_valid = datetime.strptime(s_valid, '%d/%m/%Y')
            diff    = (d_valid - d_ent).days
            return diff if diff >= 0 else None
        except Exception:
            return None

    df_sresult['trl_real_flu']   = df_sresult.apply(
        lambda r: calc_trl(r['data_de_entrada'], r['data_de_validacao_flu']), axis=1)
    df_sresult['trl_sars_cov_2'] = df_sresult.apply(
        lambda r: calc_trl(r['data_de_entrada'], r['data_de_validacao_sars']), axis=1)
    df_sresult['trl_real_rsv']   = df_sresult.apply(
        lambda r: calc_trl(r['data_de_entrada'], r['data_de_validacao_rsv']), axis=1)
...
# ============================================================================
    ...
    if df_relatorio is not None:
        ...
        seg_ant = data_inicio - timedelta(days=data_inicio.weekday() + 7)  # Segunda anterior
        sex_ant = seg_ant + timedelta(days=4)                               # Sexta anterior
        df_anterior   = df_fmt[
