...
def pagina_login():
    ...
    with col_c:
        st.markdown(
            "<div style='text-align:center;font-size:4rem;margin:1.5rem 0 0.5rem'>🦠</div>",
            unsafe_allow_html=True)
        st.markdown(
            "<h2 style='text-align:center;color:#1e3a8a;margin-bottom:0'>Dashboard IRAS</h2>"
            "<p style='text-align:center;color:#64748b;margin-bottom:1.5rem'>"
            "Gerador de Relatórios e Dados Demográficos<br>Instituto Nacional de Saúde — Moçambique</p>",
            unsafe_allow_html=True)
        ...
        st.markdown(
            ...
            "v3.1 · INS Moçambique</div>", unsafe_allow_html=True)
...
def to_date_str(value):
    """Converte qualquer formato de data para YYYY-MM-DD (uso interno)."""
...
def to_date_dmy(value):
    """Converte qualquer formato de data para DD/MM/AAAA (exibição)."""
    iso = to_date_str(value)
    if not iso:
        return None
    try:
...
def gerar_resumo_dinamico(df_atual, df_anterior, periodo_atual_str, periodo_anterior_str):
    ...
    # ── Comparação semanal ────────────────────────────────────────────────────
    # Cada vírus usa SOMENTE a sua coluna de resultado como denominador
    def _virus_stats(df, resultado_col):
        """Devolve (positivos, testados, pct) usando apenas a coluna do vírus."""
        if resultado_col not in df.columns:
            return 0, 0, 0.0
        serie = df[resultado_col].astype(str).str.strip()
        # Testados = linhas com resultado preenchido (não vazio, não '-', não nan)
        t = int((~serie.isin(['-', '', 'nan', 'None', 'none'])).sum())
        p = int(serie.str.upper().str.contains('POSITIVO', na=False).sum())
        pct = round(100 * p / t, 1) if t > 0 else 0.0
        return p, t, pct

    fa_p, fa_t, fa = _virus_stats(df_atual,    "Influenza")
    fp_p, fp_t, fp = _virus_stats(df_anterior, "Influenza")
    sa_p, sa_t, sa = _virus_stats(df_atual,    "SARS-CoV-2")
    sp_p, sp_t, sp = _virus_stats(df_anterior, "SARS-CoV-2")
    ra_p, ra_t, ra = _virus_stats(df_atual,    "RSV")
    rp_p, rp_t, rp = _virus_stats(df_anterior, "RSV")

    def seta(atual, prev):
        return "↑" if atual > prev else ("↓" if atual < prev else "→")

    def fmt_pct(p, t, pct):
        return f"{pct}% ({p}/{t})" if t > 0 else "n/d"

    resumo += (
        f"\n\nComparação com a semana anterior ({periodo_anterior_str}):\n"
        f"  • Influenza:   {fmt_pct(fp_p,fp_t,fp)} → {fmt_pct(fa_p,fa_t,fa)} {seta(fa, fp)}\n"
        f"  • SARS-CoV-2: {fmt_pct(sp_p,sp_t,sp)} → {fmt_pct(sa_p,sa_t,sa)} {seta(sa, sp)}\n"
        f"  • RSV:         {fmt_pct(rp_p,rp_t,rp)} → {fmt_pct(ra_p,ra_t,ra)} {seta(ra, rp)}"
    )
    return resumo
...
def criar_tabelas_unidades_sanitarias(doc, df):
    ...
    for cod in sorted(unidades):
        ...
        # Helper: data de testagem prioritária FLU > SARS > RSV
        def _get_dt_testagem(row_dict):
            for col in ['Data da Testagem FLU', 'Data da Testagem SARS', 'Data da Testagem RSV',
                        'data_da_testagem_flu', 'data_de_testagem_sars', 'data_da_testagem_rsv']:
                v = row_dict.get(col)
                if v and pd.notna(v) and str(v).strip() not in ['', 'None', 'nan', '-']:
                    return str(v).strip()
            return ""

        for idx, row in enumerate(df_site.itertuples(index=False), start=1):
            row_dict = df_site.iloc[idx-1].to_dict()
            ...
            cells[7].text  = _get_dt_testagem(row_dict)
            cells[8].text  = str(row[7])   # Influenza
            cells[9].text  = str(row[8])   # RSV
            cells[10].text = str(row[9])   # SARS-CoV-2
...
def process_influenza(sresult_file, demo_file, progress_callback=None, target_sheet=None):
    ...
    # ── LOCALIZAR ÂNCORAS E CALCULAR DATAS POR OFFSET FIXO ─────────────────
    # Regra confirmada no SResult real:
    #   âncora - 5 = data de VALIDAÇÃO   (mais próxima da âncora)
    #   âncora - 8 = data de TESTAGEM    (mais distante da âncora)
    #
    #   SARS : âncora secção "RTCO2-CV19R"  (col 71 no ficheiro de referência)
    #   FLU  : âncora secção "RTRP-FLA"     (col 84)
    #   RSV  : âncora secção "RSV-RSVRS"    (col 112)

    def find_col_by_sec(sec_pat, col_pat):
        for i, (s, c) in enumerate(zip(sec_fill, col_row)):
            if re.search(sec_pat, s, re.I) and re.search(col_pat, c, re.I):
                return i + 1
        return None

    # Âncoras dos resultados
    i_sars_res = find_col_by_sec(r"RTCO2",     r"Result|CV19R|SARS-CoV-2")
    i_flu_a    = find_col_by_sec(r"RTRP-FLA",  r"FLU A")
    i_flu_b    = find_col_by_sec(r"RTRP-FLB",  r"FLU B")
    i_rsv_res  = find_col_by_sec(r"RSV-RSVRS", r"RSV Result|RSV")
    i_subtipo  = find_col(r"RTRP-SUBTI", r"Subtipo")

    # Datas por offset fixo a partir das âncoras
    i_sars_valid = (i_sars_res - 5) if i_sars_res else None   # col 66
    i_sars_test  = (i_sars_res - 8) if i_sars_res else None   # col 63
    i_flu_valid  = (i_flu_a    - 5) if i_flu_a    else None   # col 79
    i_flu_test   = (i_flu_a    - 8) if i_flu_a    else None   # col 76
    i_rsv_valid  = (i_rsv_res  - 5) if i_rsv_res  else None   # col 107
    i_rsv_test   = (i_rsv_res  - 8) if i_rsv_res  else None   # col 104

    log(f"  ref={i_ref}, us={i_us}, idade={i_idade}, sexo={i_sexo}, end={i_end}")
    log(f"  lab={i_lab}, flu_a={i_flu_a}, rsv_res={i_rsv_res}, sars_res={i_sars_res}")
    log(f"  flu_test={i_flu_test}(col), flu_valid={i_flu_valid}(col)")
    log(f"  sars_test={i_sars_test}(col), sars_valid={i_sars_valid}(col)")
    log(f"  rsv_test={i_rsv_test}(col),  rsv_valid={i_rsv_valid}(col)")
    if i_ent_dt is None:
    ...
    def to_date_display(value):
        """Converte data para DD/MM/AAAA para exibição."""
        d = to_date_str(value)
        if not d:
            return None
        try:
    ...
    def get_col_display(idx):
        """Extrai coluna de data e converte para DD/MM/AAAA."""
        raw = get_col(idx)
        return raw.apply(to_date_display)
    ...
    df_sresult = pd.DataFrame({
        ...
        'data_da_colheita':      get_col(i_col_dt).apply(to_date_dmy),
        'hora_de_colheita':      hr_fmt,
        'data_de_nascimento':    get_col(i_ddn).apply(to_date_dmy),
        'idade':                 get_col(i_idade),
        'sexo':                  get_col(i_sexo),
        'residencia_bairro':     get_col(i_end),
        'data_de_entrada':       (get_col(i_ent_dt).apply(to_date_dmy)
                                  if i_ent_dt else get_col(i_col_dt).apply(to_date_dmy)),
        'data_da_testagem_flu':  get_col_display(i_flu_test),
        'data_de_validacao_flu': get_col_display(i_flu_valid),
        'data_de_testagem_sars': get_col_display(i_sars_test),
        'data_de_validacao_sars':get_col_display(i_sars_valid),
        'data_da_testagem_rsv':  get_col_display(i_rsv_test),
        'data_de_validacao_rsv': get_col_display(i_rsv_valid),
    ...
    # ── TRL automático (dias entre colheita e testagem) ──────────────────────
    def calc_trl(data_col_iso, data_test_display):
        try:
            if not data_col_iso or not data_test_display:
                return None
            d_col  = datetime.strptime(str(data_col_iso).strip()[:10], '%Y-%m-%d')
    ...
    try:
        ...
        _test_valid_cols = {'data_da_testagem_flu','data_de_validacao_flu',
                            'data_de_testagem_sars','data_de_validacao_sars',
                            'data_da_testagem_rsv','data_de_validacao_rsv'}
        for col in df_demo.columns:
            if col in _test_valid_cols:
                # Manter em DD/MM/AAAA — converter de ISO ou qualquer formato
                df_demo[col] = df_demo[col].apply(
                    lambda v: (lambda d: datetime.strptime(d,'%Y-%m-%d').strftime('%d/%m/%Y')
                               if d else None)(to_date_str(v)) if pd.notna(v) else None)
            elif 'data' in col:
                df_demo[col] = df_demo[col].apply(to_date_dmy)
...
if secao == "📊 Processamento de Dados":
    ...
    # ── Filtro de período de entrada (opcional) ─────────────────────────────
    st.markdown("#### 📆 Filtro de período")
    st.caption("Filtra os registos pela **Data de entrada** — útil para agregar apenas uma semana específica.")
    cp1, cp2 = st.columns(2)
    with cp1:
        data_ini_proc = st.date_input(
            "Data de entrada — início",
            value=date.today() - timedelta(days=6),
            min_value=date(2020, 1, 1),
            key='proc_di')
    with cp2:
        data_fim_proc = st.date_input(
            "Data de entrada — fim",
            value=date.today(),
            min_value=data_ini_proc,
            key='proc_df')
...
# ============================================================================
    ...
    if df_relatorio is not None:
        ...
        col_dt_flu      = enc(df_relatorio, 'data_da_testagem_flu',  'Data da Testagem FLU')
        col_dt_sars     = enc(df_relatorio, 'data_de_testagem_sars', 'Data de Testagem SARS')
        col_dt_rsv      = enc(df_relatorio, 'data_da_testagem_rsv',  'Data da Testagem RSV')
        ...
        def fmt_result(col):
            ...
            return df_relatorio[col].apply(_fmt)
        ...
        df_fmt = pd.DataFrame({
            ...
            "Data da Testagem FLU":  df_relatorio[col_dt_flu].fillna('').astype(str)  if col_dt_flu  else "",
            "Data da Testagem SARS": df_relatorio[col_dt_sars].fillna('').astype(str) if col_dt_sars else "",
            "Data da Testagem RSV":  df_relatorio[col_dt_rsv].fillna('').astype(str)  if col_dt_rsv  else "",
        })
        ...
        # ── Período anterior — datas manuais ──────────────────────────────
        st.markdown("#### 📅 Período de comparação (semana anterior)")
        st.caption("Defina manualmente o intervalo da semana anterior para a comparação de positividade.")
        # Sugestão automática: seg-sex da semana precedente
        _seg_sug = data_inicio - timedelta(days=data_inicio.weekday() + 7)
        _sex_sug = _seg_sug + timedelta(days=4)
        ca1, ca2 = st.columns(2)
        with ca1:
            data_ant_ini = st.date_input(
                "Semana anterior — início",
                value=_seg_sug,
                min_value=date(2020, 1, 1),
                key='rel_ant_ini')
        with ca2:
            data_ant_fim = st.date_input(
                "Semana anterior — fim",
                value=_sex_sug,
                min_value=data_ant_ini,
                key='rel_ant_fim')
        ...
        df_anterior = df_fmt[
            (df_fmt["Data de entrada"] >= pd.Timestamp(data_ant_ini)) &
            (df_fmt["Data de entrada"] <= pd.Timestamp(data_ant_fim))
        ].copy()

        periodo_actual_str   = f"{data_inicio.strftime('%d/%m/%Y')} a {data_fim.strftime('%d/%m/%Y')}"
        periodo_anterior_str = f"{data_ant_ini.strftime('%d/%m/%Y')} a {data_ant_fim.strftime('%d/%m/%Y')}"

        st.info(f"📊 Período actual: **{len(df_atual):,}** registos ({periodo_actual_str})")
        st.info(f"📊 Semana anterior: **{len(df_anterior):,}** registos ({periodo_anterior_str})")
        st.markdown("---")
...
# ============================================================================
    ...
    if df_graphs is not None:
        ...
        semanas = sorted(
            [s for s in _df_sv['semana_epidemiologica'].astype(str).unique()
             if s.strip().isdigit()],
            key=lambda x: int(x))
