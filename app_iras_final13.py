...
st.set_page_config(
    page_title="Dashboard IRAS: Gerador de Relatórios e Dados Demográficos",
    page_icon="🦠",
    layout="wide"
)
...
# ============================================================================
# SISTEMA DE LOGIN  (modificação 1)
# ============================================================================
...
def pagina_login():
    ...
    with col_c:
        ...
        st.markdown(
            '<p class="login-title">Dashboard IRAS</p>'
            '<p class="login-sub">Gerador de Relatórios e Dados Demográficos<br>'
            'Instituto Nacional de Saúde — Moçambique</p>',
            unsafe_allow_html=True)

        with st.form("form_login", clear_on_submit=False):
            usuario  = st.text_input("👤 Utilizador", placeholder="Introduza o utilizador")
            senha    = st.text_input("🔒 Senha",      placeholder="Introduza a senha", type="password")
        ...
        st.markdown(
            ...
            "v3.1 · INS Moçambique · Vigilância Sentinela de IRAs</div>",
            unsafe_allow_html=True)
...
def to_date_display(value):
    """Converte data para formato de exibição DD/MM/AAAA."""
    d = to_date_str(value)
    if not d:
        return ""
    try:
...
def classificar_unidade_sanitaria(codigo, local_colheita=None, idade=None):
    ...
    codigo = next((p for p in parts if re.match(r'^IRAS\d', re.sub(r'[^A-Z0-9]','',p))), raw)
...
def _get_semana_anterior(data_inicio, data_fim):
    """Retorna (seg, sex) da semana anterior (segunda a sexta) relativa ao período actual."""
    # Encontrar a segunda-feira da semana actual
    # data_inicio pode não ser segunda — ir para a segunda-feira mais próxima anterior
    weekday_ini = data_inicio.weekday()  # 0=Mon
    seg_actual  = data_inicio - timedelta(days=weekday_ini)
    seg_ant     = seg_actual - timedelta(days=7)
    sex_ant     = seg_ant + timedelta(days=4)
    return seg_ant, sex_ant
...
def gerar_resumo_dinamico(df_atual, df_anterior, periodo_atual_str, periodo_anterior_str):
    ...
    CORRIGIDO: totais calculados por coluna de data de testagem de cada vírus
               (não pelo total de linhas do período).
    """
    total = len(df_atual)
    ...
    # ---------- helpers corrigidos -------------------------------------------
    def _tested_virus(series):
        """Conta amostras testadas: resultado preenchido (não '-', não vazio, não nan)."""
        return int(
            series.astype(str).str.strip()
                  .replace({'': 0, '-': 0, 'nan': 0, 'none': 0, 'None': 0})
                  .apply(lambda x: 0 if str(x) in ['0','nan','none','None','-',''] else 1)
                  .sum()
        )

    def _tested_by_date(df_u, date_col):
        """
        Conta amostras testadas usando a coluna de data de testagem de cada vírus
        como denominador real — alinha com o que foi efectivamente processado.
        """
        if date_col not in df_u.columns:
            return len(df_u)
        return int(df_u[date_col].apply(
            lambda x: 1 if (pd.notna(x) and str(x).strip() not in ['','nan','None','-'])
            else 0).sum())
    ...
    for cod_un, nome_un in sorted(unidades.items()):
        ...
        # ── INFLUENZA — denominador = amostras com data de testagem FLU ──────
        flu_test = _tested_by_date(df_u, "Data da Testagem FLU")
        # fallback: se coluna de data não existe, usar resultado
        if flu_test == 0 and "Influenza" in df_u.columns:
            flu_test = _tested_virus(df_u["Influenza"])
        flu_pos = _is_pos(df_u["Influenza"]) if "Influenza" in df_u.columns else 0
        ...
        # ── RSV — denominador = amostras com data de testagem RSV ─────────────
        rsv_test = _tested_by_date(df_u, "Data da Testagem RSV")
        if rsv_test == 0 and "RSV" in df_u.columns:
            rsv_test = _tested_virus(df_u["RSV"])
        rsv_pos  = _is_pos(df_u["RSV"]) if "RSV" in df_u.columns else 0
        ...
        # ── SARS-CoV-2 — denominador = amostras com data de testagem SARS ─────
        sars_test = _tested_by_date(df_u, "Data da Testagem SARS")
        if sars_test == 0 and "SARS-CoV-2" in df_u.columns:
            sars_test = _tested_virus(df_u["SARS-CoV-2"])
        sars_pos  = _is_pos(df_u["SARS-CoV-2"]) if "SARS-CoV-2" in df_u.columns else 0
    ...
    # ── Comparação com semana anterior (Segunda a Sexta) ─────────────────────
    # Calcular taxas usando denominadores por data de testagem de cada vírus
    def taxas_corretas(df):
        def _pct_virus(resultado_col, data_test_col):
            if resultado_col not in df.columns:
                return 0.0
            t = _tested_by_date(df, data_test_col) if data_test_col in df.columns else \
                _tested_virus(df[resultado_col])
            p = _is_pos(df[resultado_col]) if resultado_col in df.columns else 0
            return round(100 * p / t, 1) if t > 0 else 0.0

        return (
            _pct_virus("Influenza",   "Data da Testagem FLU"),
            _pct_virus("SARS-CoV-2", "Data da Testagem SARS"),
            _pct_virus("RSV",        "Data da Testagem RSV"),
        )

    fa, sa, ra = taxas_corretas(df_atual)
    fp, sp, rp = taxas_corretas(df_anterior)
...
def criar_tabelas_unidades_sanitarias(doc, df):
    ...
    MODIFICADO: inclui coluna 'Data de Testagem' (FLU > SARS > RSV).
    """
    ...
    # Helper para obter data de testagem prioritária
    def get_data_testagem(row):
        for col in ['Data da Testagem FLU', 'Data da Testagem SARS', 'Data da Testagem RSV']:
            v = row.get(col)
            if pd.notna(v) and str(v).strip() not in ['', '-', 'nan', 'None']:
                # já deve estar em DD/MM/AAAA pois foi convertido no df_fmt
                return str(v).strip()
        return ""
    ...
    for cod in sorted(unidades):
        ...
        # 11 colunas: +Data de Testagem
        table = doc.add_table(rows=1, cols=11)
        table.style     = 'Table Grid'
        table.alignment = WD_TABLE_ALIGNMENT.CENTER

        hdrs = ["Ord.", "Código", "Sexo", "Idade", "Residência/Bairro",
                "Data de Colheita", "Tipo de Amostra", "Data de Testagem",
        ...
        for idx, row in df_site.iterrows():
            ...
            cells[0].text  = str(df_site.index.get_loc(idx) + 1)
            cells[1].text  = str(row.get('Código', ''))
            cells[2].text  = str(row.get('Sexo', ''))
            cells[3].text  = str(row.get('Idade', ''))
            cells[4].text  = str(row.get('Residência/Bairro', ''))
            dc = row.get('Data da Colheita')
            cells[5].text  = dc.strftime('%d/%m/%Y') if pd.notna(dc) and hasattr(dc,'strftime') \
                             else str(dc) if pd.notna(dc) else ""
            cells[6].text  = "Swab nasal/orofaríngeo"
            cells[7].text  = get_data_testagem(row)
            cells[8].text  = str(row.get('Influenza', '-'))
            cells[9].text  = str(row.get('RSV', '-'))
            cells[10].text = str(row.get('SARS-CoV-2', '-'))
...
# ============================================================================
# PROCESSAMENTO INFLUENZA  (modificação 3 — datas corrigidas + TRL + filtro)
# ============================================================================

def process_influenza(sresult_file, demo_file, progress_callback=None,
                      target_sheet=None, data_inicio=None, data_fim=None):
    logs = []
    def log(msg):
        logs.append(msg)
    ...
    i_prov   = (find_col("NOMES PARA", r"Província") or find_col("NOMES PARA", r"Provincia"))
    ...
    # ── DATAS DE TESTAGEM E VALIDAÇÃO POR VÍRUS (CORRIGIDO) ─────────────────
    # Estrutura SResult: secção do vírus → [CódDeTeste, EntrPor, dd-mm-aaaa(entrada),
    #                                        hh:mm, VerifPor, dd-mm-aaaa(testagem),
    #                                        hh:mm, Máquina, LabTestado, #Repets]
    # A 1ª ocorrência de dd-mm-aaaa após a secção = data de entrada no teste
    # A 2ª ocorrência de dd-mm-aaaa após a secção = data de TESTAGEM (validação da máquina)
    # ─────────────────────────────────────────────────────────────────────────

    # SARS-CoV-2 (secção RT-PCR SARS-COV2)
    i_sars_test  = find_col(r"RT-PCR SARS-COV|RT-PCR SARS|RTRP-SARS", r"^dd-mm-aaaa$", 1)
    i_sars_valid = find_col(r"RT-PCR SARS-COV|RT-PCR SARS|RTRP-SARS", r"^dd-mm-aaaa$", 2)
    i_sars_res   = (find_col(r"RTCO2|RTRP-SARS", r"^SARS-CoV-2$|^Result$|CV19R") or
                    find_col(r"RTCO2",             r"Result"))

    # Influenza (secção RT-PCR INFLUENZA)
    i_flu_test   = find_col(r"RT-PCR INFLU|RTRP-BYAGA|RT-PCR INFLU", r"^dd-mm-aaaa$", 1)
    i_flu_valid  = find_col(r"RT-PCR INFLU|RTRP-BYAGA|RT-PCR INFLU", r"^dd-mm-aaaa$", 2)
    i_flu_a      = find_col(r"RTRP-FLA",   r"FLU A")
    i_flu_b      = find_col(r"RTRP-FLB",   r"FLU B")
    i_subtipo    = find_col(r"RTRP-SUBTI", r"Subtipo")

    # RSV (secção VÍRUS SINCICIAL / RSV)
    ...
    log(f"  flu_test={i_flu_test}, flu_valid={i_flu_valid}")
    log(f"  sars_test={i_sars_test}, sars_valid={i_sars_valid}, sars_res={i_sars_res}")
    log(f"  rsv_test={i_rsv_test},  rsv_valid={i_rsv_valid},  rsv_res={i_rsv_res}")
    ...
    def get_col_as_date_display(idx):
        """Extrai coluna de data e converte para DD/MM/AAAA."""
        raw = get_col(idx)
        def _fmt(v):
    ...
    # Datas de testagem e validação → formato DD/MM/AAAA
    dt_flu_test  = get_col_as_date_display(i_flu_test)
    dt_flu_valid = get_col_as_date_display(i_flu_valid)
    dt_sars_test = get_col_as_date_display(i_sars_test)
    dt_sars_valid= get_col_as_date_display(i_sars_valid)
    dt_rsv_test  = get_col_as_date_display(i_rsv_test)
    dt_rsv_valid = get_col_as_date_display(i_rsv_valid)
    ...
    df_sresult = pd.DataFrame({
        ...
        # Datas em DD/MM/AAAA
        'data_da_testagem_flu':   dt_flu_test,
        'data_de_validacao_flu':  dt_flu_valid,
        'data_de_testagem_sars':  dt_sars_test,
        'data_de_validacao_sars': dt_sars_valid,
        'data_da_testagem_rsv':   dt_rsv_test,
        'data_de_validacao_rsv':  dt_rsv_valid,
    ...
    # ── 4. COLUNAS DERIVADAS ────────────────────────────────────────────────
    log("[4/7] A calcular subtipos, resultados e TRLs...")
    ...
    # ── TRL automático ──────────────────────────────────────────────────────
    # TRL = dias entre data_da_colheita e data_da_testagem
    def calc_trl(data_col_str, data_test_display):
        """Calcula TRL em dias entre colheita (YYYY-MM-DD) e testagem (DD/MM/AAAA)."""
        try:
            if not data_col_str or not data_test_display:
                return None
            d_col  = datetime.strptime(str(data_col_str).strip()[:10], '%Y-%m-%d')
            d_test = datetime.strptime(str(data_test_display).strip(), '%d/%m/%Y')
            diff   = (d_test - d_col).days
            return diff if diff >= 0 else None
        except Exception:
            return None

    df_sresult['trl_real_flu']   = df_sresult.apply(
        lambda r: calc_trl(r['data_da_colheita'], r['data_da_testagem_flu']), axis=1)
    df_sresult['trl_sars_cov_2'] = df_sresult.apply(
        lambda r: calc_trl(r['data_da_colheita'], r['data_de_testagem_sars']), axis=1)
    df_sresult['trl_real_rsv']   = df_sresult.apply(
        lambda r: calc_trl(r['data_da_colheita'], r['data_da_testagem_rsv']), axis=1)
    ...
    def padronizar_codigo(codigo):
        ...
        if len(all_digits) == 10 and int(all_digits[:2]) in _KNOWN_2D:
    ...
    try:
        ...
        # Datas no demográfico: converter para DD/MM/AAAA nas colunas de testagem/validação
        date_cols_display = ['data_da_testagem_flu','data_de_validacao_flu',
                             'data_de_testagem_sars','data_de_validacao_sars',
                             'data_da_testagem_rsv','data_de_validacao_rsv']
        date_cols_iso = ['data_da_colheita','data_de_nascimento','data_de_entrada']
        for col in df_demo.columns:
            if col in date_cols_display:
                df_demo[col] = df_demo[col].apply(
                    lambda v: to_date_display(v) if pd.notna(v) else None)
            elif col in date_cols_iso or 'data' in col:
    ...
    # ── 7. COMBINAR E FILTRAR POR PERÍODO ────────────────────────────────────
    ...
    if data_inicio and data_fim:
        dt_ref = pd.to_datetime(combined['data_de_entrada'], errors='coerce')
        mask   = (dt_ref >= pd.Timestamp(data_inicio)) & (dt_ref <= pd.Timestamp(data_fim))
        n_antes = len(combined)
        combined = combined[mask]
        log(f"  Filtro período {data_inicio}→{data_fim}: {n_antes} → {len(combined)} registos")
    ...
    col_order = [
        ...
        'data_da_testagem_flu','data_de_validacao_flu',
        'data_de_testagem_sars','data_de_validacao_sars',
        'data_da_testagem_rsv','data_de_validacao_rsv',
...
def process_rsv(sresult_file, demo_file, progress_callback=None,
                target_sheet=None, data_inicio=None, data_fim=None):
    logs = []
    def log(msg):
        logs.append(msg)
    ...
    # Converter datas: colheita/entrada → ISO, testagem/validação → DD/MM/AAAA
    test_valid_cols = ['data_da_testagem_rsv','data_de_validacao_rsv',
                       'data_da_testagem_flu','data_de_validacao_flu',
                       'data_de_testagem_sars','data_de_validacao_sars']
    for col in df_sr.columns:
        if 'data' in col:
            if col in test_valid_cols:
                df_sr[col] = df_sr[col].apply(cvt_date_display)
            else:
                df_sr[col] = df_sr[col].apply(cvt_date_iso)
    ...
    try:
        ...
        df_demo = pd.read_excel(demo_file, sheet_name=target_sheet, dtype=str)
        df_demo = clean_column_names(df_demo)
        test_valid_cols_demo = ['data_da_testagem_flu','data_de_validacao_flu',
                                'data_de_testagem_sars','data_de_validacao_sars',
                                'data_da_testagem_rsv','data_de_validacao_rsv']
        for col in df_demo.columns:
            if col in test_valid_cols_demo:
                df_demo[col] = df_demo[col].apply(
                    lambda v: to_date_display(v) if pd.notna(v) else None)
            elif 'data' in col:
                df_demo[col] = df_demo[col].apply(to_date_str)
    ...
    if data_inicio and data_fim:
        ...
        n_antes = len(combined)
        combined = combined[mask]
        log(f"  Filtro período: {n_antes} → {len(combined)} registos")
...
st.sidebar.caption(f"👤 {st.session_state.get('usuario_nome','')}")
...
""")
st.sidebar.markdown("---")
st.sidebar.caption(f"v3.1 · INS · {datetime.now().strftime('%Y-%m-%d')}")
...
if secao == "📊 Processamento de Dados":
    ...
    st.markdown("#### 📆 Filtro de período (opcional)")
    st.caption("Se definido, apenas os registos com 'Data de entrada' neste intervalo serão incluídos no Excel exportado.")
    usar_filtro_periodo = st.checkbox("Filtrar por período de entrada", value=False, key='chk_filtro')
    data_ini_proc = data_fim_proc = None
    if usar_filtro_periodo:
        cp1, cp2 = st.columns(2)
        with cp1:
            data_ini_proc = st.date_input("Data inicial", value=date.today() - timedelta(days=6),
                                          key='proc_di')
        with cp2:
            data_fim_proc = st.date_input("Data final",   value=date.today(),
                                          min_value=data_ini_proc, key='proc_df')
        ...
        if not sresult_file or not demo_file:
            ...
            try:
                if tipo_analise == "Influenza":
                    ...
                    df_result, logs = process_influenza(
                        sresult_file, demo_file, update_progress, selected_sheet,
                        data_ini_proc, data_fim_proc)
                else:
                    progress_bar.progress(15, text="A processar RSV...")
                    df_result, logs = process_rsv(
                        sresult_file, demo_file, update_progress, selected_sheet,
                        data_ini_proc, data_fim_proc)
...
# ============================================================================
    ...
    if df_relatorio is not None:
        ...
        # Colunas de data de testagem por vírus
        col_dt_flu   = enc(df_relatorio, 'data_da_testagem_flu',  'Data da Testagem FLU', 'Data Testagem FLU')
        col_dt_sars  = enc(df_relatorio, 'data_de_testagem_sars', 'Data de Testagem SARS','Data Testagem SARS')
        col_dt_rsv   = enc(df_relatorio, 'data_da_testagem_rsv',  'Data da Testagem RSV', 'Data Testagem RSV')
        ...
        def _date_col(col):
            """Devolve coluna já em DD/MM/AAAA ou vazia."""
            if col is None:
                return pd.Series([""]*len(df_relatorio))
            return df_relatorio[col].apply(
        ...
        df_fmt = pd.DataFrame({
            ...
            # Datas de testagem por vírus — para resumo e tabela
            "Data da Testagem FLU":  _date_col(col_dt_flu),
            "Data da Testagem SARS": _date_col(col_dt_sars),
            "Data da Testagem RSV":  _date_col(col_dt_rsv),
        })

        # ── Selecção de período actual ────────────────────────────────────
        ...
        # ── Semana anterior: Segunda a Sexta da semana precedente ─────────
        seg_ant, sex_ant = _get_semana_anterior(data_inicio, data_fim)
        df_anterior = df_fmt[
            (df_fmt["Data de entrada"] >= pd.Timestamp(seg_ant)) &
            (df_fmt["Data de entrada"] <= pd.Timestamp(sex_ant))
        ].copy()

        periodo_actual_str   = f"{data_inicio.strftime('%d/%m/%Y')} a {data_fim.strftime('%d/%m/%Y')}"
        periodo_anterior_str = f"{seg_ant.strftime('%d/%m/%Y')} a {sex_ant.strftime('%d/%m/%Y')}"

        st.info(f"📊 Período actual: **{len(df_atual):,}** registos ({periodo_actual_str})")
        st.info(f"📊 Semana anterior (Seg–Sex): **{len(df_anterior):,}** registos ({periodo_anterior_str})")
        st.markdown("---")
...
# ============================================================================
    ...
    if df_graphs is not None:
        ...
        if soma > tot_pos:
            st.info(f"ℹ️ Existem **{soma-tot_pos}** co-infecção(ões).")
        ...
        if 'codigo_do_site' in df_graphs.columns:
            ...
            if rem > 0:
                st.caption(f"ℹ️ {rem} amostra(s) sem código de sítio válido excluída(s).")
        ...
        if not semanas:
            ...
            fig_sup.add_trace(go.Bar(x=x_vals, y=tot_vals, name='Amostras testadas',
                marker_color='#d4af37', opacity=0.55), secondary_y=False)
            fig_sup.add_trace(go.Scatter(x=x_vals, y=flu_vals, name='% Influenza',
                mode='lines+markers', connectgaps=False,
                line=dict(color='#1e3a8a', width=3), marker=dict(size=8)), secondary_y=True)
            fig_sup.add_trace(go.Scatter(x=x_vals, y=sar_vals, name='% SARS-CoV-2',
                mode='lines+markers', connectgaps=False,
                line=dict(color='#dc2626', width=3), marker=dict(size=8)), secondary_y=True)
            fig_sup.update_layout(
                title='Amostras testadas e positividade semanal — Influenza e SARS-CoV-2',
                xaxis=dict(title='Semana epidemiológica', tickangle=-45,
...
st.markdown("---")
st.caption(
    f"**Dashboard IRAS: Gerador de Relatórios e Dados Demográficos — INS Moçambique**  "
    f"v3.1 · Influenza · RSV · SARS-CoV-2 · "
